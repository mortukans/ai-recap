...
// Returns { language: string|null, segments: [{ start, end, text }] } with times relative to the chunk.
// The audio is processed in memory and never stored server-side (Arch §16: no user content at rest).
import { TRANSCRIPTION_MODEL, admin, callOpenRouter, json, recordUsage, requireUnlimited, requireUser } from '../_shared/hosted.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      const parsed = JSON.parse(jsonText) as { language?: string | null; segments?: { start?: number; end?: number; text?: string }[] };
      return json({
        language: parsed.language ?? null,
        segments: (parsed.segments ?? [])
          .map((s) => ({ start: Number(s.start ?? 0), end: Number(s.end ?? duration), text: (s.text ?? '').trim() }))
          .filter((s) => s.text.length > 0),
      });
    } catch {
      return json({ language: null, segments: trimmed ? [{ start: 0, end: duration, text: trimmed }] : [] });
    }
  } catch (e) {
