...
_Last updated: 2026-09-25 — **1.0 SUBMITTED TO APP REVIEW** (build 1.0.0 (33) + both IAPs + subscription group); post-submission work underway. Owner: Martins Mortukans._

**Post-submission (2026-09-25):**
- `main`: direct App Store upload from CI via `altool` when ASC API key secrets are set (skips the ~3 h EAS Submit queue; falls back automatically) — `docs/CI_DIRECT_UPLOAD.md`; expanded GitHub Pages support page; English/generic store screenshots (`design/store-screenshots/**`, generated by `make_shots.py`).
- Branch `release/1.0.1`: anonymous crash/error diagnostics (`log-error` Edge Function + `error_events`, verified live), ErrorBoundary, privacy-policy disclosure, version `1.0.1`. See `docs/RELEASE_1_0_1.md` (App Privacy label update required before submitting).
